...
- 本仓库为对比实验版本：`JianingZhangnan/AISE-copy`（用于与另一个 agent 在 `JianingZhangnan/AISE` 上的实现进行比较）。
- 最终提交平台：**NJU Git**（课程明确要求，见 `course_resource\AI4SE_Final_Project_通用要求.md` §五）。
- **本地仓库**（`d:\cs\AISE - Copy`，当前目录）作为开发源；通过 GitHub 仓库 `JianingZhangnan/AISE-copy` 进行中间备份和 PR 评审。
- 完整提交前应以本地仓库为源，**镜像到 NJU Git**（保留完整 commit 历史、文档、CI 配置和交付物），并在 `AGENT_LOG.md` / `SPEC_PROCESS.md` 中记录平台切换原因与关键操作。
- 不论最终提交平台如何，仓库纪律、凭据安全、CI、SPEC / PLAN / SPEC_PROCESS / AGENT_LOG 等交付要求保持不变。

## CI / CD 要求（重要更正）

**课程通用要求 §4.8 / §五明确：CI 是 GitHub Actions，不是 GitLab CI。**

- **必须**配置 GitHub Actions（`.github/workflows/ci.yml`），每次 push 自动运行测试。
- 课程通用要求 §五还要求 **`.gitlab-ci.yml`** 中包含名为 `unit-test` 的 job（用于 NJU Git 的 CI 校验）。
- 因此本项目应**同时**提供两个 CI 配置：
  - `.github/workflows/ci.yml`：用于 GitHub 仓库的日常 CI。
  - `.gitlab-ci.yml`：用于 NJU Git 的 CI 校验（最少包含 `unit-test` job）。
- CI/CD 最后一次执行**必须**是 pass 状态（课程硬性要求）。
